...
const openPickHandler = () => {
    ...
    const displayNum = pickArr.filter(con =>
        getComputedStyle(con).display === "block"
    )

    lastDisplayIndex = displayNum.length;

    for (let i = lastDisplayIndex; i < lastDisplayIndex + multiple; i++) {
        if (i === pickArr.length) {
            break;
        }
        pickArr[i].style.display = "block";
    }

    lastDisplayIndex += multiple; //마지막 사진나오는 동시에 바로 버튼 없애려고 한번 더 할당하는거

    if (lastDisplayIndex >= pickArr.length) {
...
window.addEventListener("resize", () => {
    ...
    timer = setTimeout(() => {
        ...
        if (window.innerWidth < 426) {
            multiple = 1;
            if (lastDisplayIndex === 4) {
                pickArr[1].style.display = "block"; //강제보정 ^...^
            }
            if (lastDisplayIndex >= 5) {
                pickArr[2].style.display = "block"; //문제 강제보정,,,
            }
            if (lastDisplayIndex === 6) {
                pickArr.map((con) => { con.style.display = "block" }) // 강제보정,,,,
            }
        } else if (window.innerWidth < 769) {
            multiple = 2;
            if (lastDisplayIndex % 2 === 1) { pickArr[lastDisplayIndex].style.display = "block" };
            if (lastDisplayIndex >= 5) {
                pickArr[2].style.display = "block"; //문제 강제보정,,,
            }
        } else {
            multiple = 3;
            if (lastDisplayIndex > 3) { pickArr.map((con) => { con.style.display = "block" }) }
        }
    }, 200)
    if (lastDisplayIndex >= pickArr.length) {
        openPick.textContent = "마지막 글입니다."
...
//지금 상황으로는 원래 스크립트가 최선인듯.. 왜없어지는지 모르겠음
...
$(function () {
    ...
    $(".imgInfo.info02").on("mouseenter", function () {
        $(".infoText.info02").css("opacity", 1)
            .css("bottom", "55%").css("left", "30%")
    })
    ...
    //pc화면 lnb 리스트 내려오게 
    $(".pcLnb li").hover(function () {
        const length = $(this).find(".hideLnb").children().length - 1;
        $(this).find(".hideLnb").css("height", `${45 * length}px`);
    }, function () {
